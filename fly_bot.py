...
if not GOOGLE_CREDENTIALS_JSON:
    raise ValueError("Biến môi trường GOOGLE_CREDENTIALS bị thiếu.")
...
@app_commands.describe(ngay="Ngày (VD: 20/7/2025)", kp="Số KP (VD: 200m, 2b, 1.5b)")
async def war_kp(interaction: discord.Interaction, ngay: str, kp: str):
    if not await has_pilot_role(interaction):
        await interaction.response.send_message("❌ Bạn cần role **Pilot** để dùng lệnh này.", ephemeral=True)
        return
    if not kp.lower().endswith(("m", "b")):
        await interaction.response.send_message("❌ KP phải có 'm' hoặc 'b' (VD: 200m, 2b).", ephemeral=True)
        return
    await interaction.response.send_message("Gửi ảnh KP ban đầu, KP lúc hoàn thành, Kill và Heal trước sau, nhắn `done` khi xong.")
    user_tasks[interaction.user.id] = {
        "sheet": "War - KP",
