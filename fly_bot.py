...
# ------------ KÉO MAN -----------------
@tree.command(name="keo-man",
              description="Ghi chú Kéo Man Rợ (ngày + số lượng)")
@app_commands.describe(ngay="Ngày (VD: 20/7/2025)",
                       so_luong="Số lượng man rợ đã kéo")
async def keo_man(interaction: discord.Interaction, ngay: str, so_luong: int):
    if not await has_pilot_role(interaction):
        await interaction.response.send_message(
            "❌ Bạn cần role **Pilot** để dùng lệnh này.", ephemeral=True)
        return
    await interaction.response.send_message(
        "Gửi ảnh chuỗi man rợ, nhắn `done` khi xong.")
...
# ------------ WAR GIỜ -----------------
@tree.command(name="war-gio",
              description="Ghi chú WAR kiểu Giờ (ngày + giờ + phút)")
@app_commands.describe(ngay="Ngày (VD: 20/7/2025)",
                       gio="Số giờ tham gia",
                       phut="Số phút tham gia")
async def war_gio(interaction: discord.Interaction, ngay: str, gio: int,
                  phut: int):
    if not await has_pilot_role(interaction):
        await interaction.response.send_message(
            "❌ Bạn cần role **Pilot** để dùng lệnh này.", ephemeral=True)
    ...
    if gio < 0 or phut < 0 or (gio == 0 and phut == 0):
        await interaction.response.send_message(
            "❌ Phải nhập giờ hoặc phút > 0.", ephemeral=True)
        return
    await interaction.response.send_message(
        "Gửi ảnh KP ban đầu, KP lúc hoàn thành, Kill và Heal trước sau, nhắn `done` khi xong."
    )
...
# ------------ WAR KP -----------------
@tree.command(name="war-kp",
              description="Ghi chú WAR kiểu KP (ngày + KP 200m, 1b,...)")
@app_commands.describe(ngay="Ngày (VD: 20/7/2025)",
                       kp="Số KP (VD: 200m, 2b, 1.5b)")
async def war_kp(interaction: discord.Interaction, ngay: str, kp: str):
    if not await has_pilot_role(interaction):
        await interaction.response.send_message(
            "❌ Bạn cần role **Pilot** để dùng lệnh này.", ephemeral=True)
        return
    if not kp.lower().endswith(("m", "b")):
        await interaction.response.send_message(
            "❌ KP phải có 'm' hoặc 'b' (VD: 200m, 2b).", ephemeral=True)
        return
    await interaction.response.send_message(
        "Gửi ảnh KP ban đầu, KP lúc hoàn thành, Kill và Heal trước sau, nhắn `done` khi xong."
    )
...
@app_commands.describe(ngay="Ngày (VD: 20/7/2025)",
                       gio="Giờ (>= 0)",
                       phut="Phút (>= 0)")
async def be_tho(interaction: discord.Interaction,
                 ngay: str,
                 gio: int = 0,
                 phut: int = 0):
    if not await has_pilot_role(interaction):
        ...
        return
    if gio < 0 or phut < 0:
        await interaction.response.send_message("❌ Giờ và phút không được âm.",
                                                ephemeral=True)
        return
    if gio == 0 and phut == 0:
        await interaction.response.send_message(
    ...
    append_to_sheet("Bệ thờ", [
        ngay, interaction.user.name, gio, phut, f"#{interaction.channel.name}",
        time_str
    ])
...
async def ark(interaction: discord.Interaction, ngay: str):
    ...
    time_str = get_vietnam_time()
    append_to_sheet("Ark", [
        ngay, interaction.user.name, f"#{interaction.channel.name}", time_str
    ])
...
async def xem_sheet(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message(
            "❌ Bạn không đủ quyền để xem Google Sheet này.", ephemeral=True)
        return
    await interaction.response.send_message(f"📄 Link tổng hợp: {LINK_SHEET}",
                                            ephemeral=True)
    ...
    if user_id in user_tasks:
        ...
        if message.content.strip().lower() == "done":
            links = [
                ...
                for msg in task["images"]
            ]
            time_str = get_vietnam_time()
            append_to_sheet(task["sheet"], [
                task["ngay"], message.author.name, task["so_luong"],
                f"#{message.channel.name}",
                ", ".join(links) if links else "Không gửi ảnh", time_str
            ])
            await message.channel.send(
                "✅ Đã ghi nhận xong và lưu vào Google Sheet.")
